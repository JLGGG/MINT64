...
//Read the current time saved by RTC controller from CMOS memory.
...
//Read the current date saved by RTC controller from CMOS memory.
...
//Returns the string of that day using a value of day.
char* kConvertDayOfWeekToString(BYTE bDayOfWeek)
{
	...
	//Returns error if exceed a range of day of the week.
	if(bDayOfWeek >= 8)
	{
